...
export default class auth extends base {
  ...
  static async login() {
    ...
    if (token != null && token != '') {
      try {
        await this.checkLoginCode(token);
      } catch (e) {
        console.warn('check login code fial', token);
        await this.doLogin();
      }
    } else {
      console.warn('login code not exists', token);
      await this.doLogin();
    }
  }
  ...
  static async session(jsCode) {
    ...
    const url = `http://localhost:8080/customer/auth/token?login_code=${jsCode}&app_code=${shopCode}`;
