...
export default class config extends base {
  ...
  static layout(pageId) {
    ...
    const url = `http://localhost:8080/customer/layout/page/${pageId}`;
  ...
  static init () {
    ...
    const url = `http://localhost:8080/customer/shops/full`;
  ...
  static processComponents (components) {
    return components
      .map(component => {
        ...
        // 处理内嵌数据
        if (component.data&&Validate.isJsonString(component.data)) {
