...
#added by yinzhixin at 2017-07-18
try:
    from runner_settings import INFLUXDB_INFO, SAVE_DATA_INTERVAL
    from .records import LastRecord
except ImportError:
    pass
RECORD = None
...
class LocustRunner(object):
    ...
    def stop(self):
        ...
        if self.hatching_greenlet and not self.hatching_greenlet.ready():
            self.hatching_greenlet.kill(block=False, timeout=5)
        self.locusts.kill(block=False, timeout=5)
...
class MasterLocustRunner(DistributedLocustRunner):
    ...
    def start_hatching(self, locust_count, hatch_rate):
        ...
        try:
            global RECORD
            RECORD = LastRecord()
            RECORD.start_record(locust_count, hatch_rate)
            gevent.spawn(self.save_chart_data, RECORD.pk_id).link_exception(callback=self.noop)
            #adde by yinzhixin at 2017-11-02
            start_analysis_url = "http://172.16.16.72:5001/start"
            gevent.spawn(requests.get, start_analysis_url).link_exception(callback=self.noop)
        except:
            logger.error("inster db error", exc_info=True)
    ...
    def save_chart_data(self, mysql_id):
        ...
        try:
            error_num = 0
            insert_interval = SAVE_DATA_INTERVAL()
    ...
    def stop(self):
        ...
        #modified by yinzhixin at 2017-12-19
        try:
            stop_analysis_url = "http://172.16.16.72:5001/pause"
            gevent.spawn(requests.get, stop_analysis_url).link_exception(callback=self.noop)
            def async_stop_record():
                global RECORD
                timer = gevent.Timeout(10)
                timer.start()
                try:
                    while True:
                        gevent.sleep(1.0)
                        if self.state == STATE_STOPPED:
                            fail_ratio = round(self.stats.aggregated_stats("Total").fail_ratio, 4)
                            total_rps = round(self.stats.aggregated_stats("Total").current_rps, 1)
                            RECORD.stop_record(fail_ratio, total_rps)
                            logger.info("Successfully save locust record-{}!".format(RECORD.pk_id))
                            break
                    ...
                    if t is timer:
                        ...
                        total_rps = round(self.stats.aggregated_stats("Total").current_rps, 1)
                        RECORD.stop_record(fail_ratio, total_rps)
                        logger.error("Save locust record 10s timeout, save data under running status")
                finally:
                    RECORD = None
                    timer.cancel()
                    logger.info("Save locust record thread stoped!")
            gevent.spawn(async_stop_record).link_exception(callback=self.noop)
            logger.info("Save locust record thread started!")
        except Exception as e:
            logger.error("Save locust record error", exec_info=True)      
    ...
    def client_listener(self):
        while True:
            ...
            if msg.type == "client_ready":
                ...
                logger.debug("Client %r reported as ready. Currently %i clients ready to swarm." % (id, len(self.clients.ready)))
                ...
                if len(self.clients.hatching + self.clients.running) == 0:
                    self.state = STATE_STOPPED
                logger.debug("Removing %s client from running clients" % (msg.node_id))
            elif msg.type == "stats":
                ...
                if msg.node_id in self.clients:
                    del self.clients[msg.node_id]
                    logger.debug("Client %r quit. Currently %i clients connected." % (msg.node_id, len(self.clients.ready)))
