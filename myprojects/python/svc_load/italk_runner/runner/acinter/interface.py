...
class acsim(object):
    ...
    def __init__(self, config_dict={}):
        self.__runner_config = config_dict
        # the connection object to italk server.
    ...
    def wait_callback_value(self, commandID, timeout):
        ...
        try:
            ...
            timer.start()
            while True:
                if commandID in self.callback_dict:
                    result = self.callback_dict.pop(commandID)
                    return result
                gevent.sleep(0.01)       
    ...
    def callback_data(self, commandID=None, data=None):
    ...
    def disconnect_italk(self):
        self.__connection.closeSocket()
        self.__connection = None
        self.callback_dict = {}

    def connect_lbs(self, ip=None, port=None):
        if self.__connection is None:
            self.__connection = MainSocket(callback_func=self.callback_data)
            self.__connection.init(host=ip, port=port)
    ...
    def connect_acc(self, ip=None, port=None, ClientType=1, ClientVer=b'v1.00.0001',
        ...
        '''
        self.__connection.build_socket_to_acc(acc_ip=ip, acc_port=port)
        if self.__connection is not None:
            data = {
    ...
    def enter_class_complete(self, SID=1, CID=None, CourseID=1):
        if CID is None:
            CID = self.__runner_config.get('cid')
        if self.__connection is not None:
            data = {
                "SID": int(SID),
                "CID": int(CID),
...
#code below added by yinzhixin at 2017-03-27
#--------------------------start----------------------------
...
if __name__ == '__main__':
    ac = acsim()
    ac.connect_italk(lbs_ip='172.16.0.115', lbs_port=6000)
    ac.login()
    ac.login_complete()
    ac.enter_classroom(CID=666)
    ac.enter_class_complete(CID=666)
    ac.chat_in_classroom(CID=666)
    ac.change_textbook_page(CID=666)
    ac.add_textbook(CID=666)
    ac.modify_textbook(CID=666)
    ac.delete_textbook(CID=666)
    ac.clear_textbook(CID=666)
    ac.add_white_board(CID=666)
    ac.get_white_board(CID=666)
    ac.modify_white_board(CID=666)
    ac.delete_white_board(CID=666)
    ac.clear_white_board(CID=666)
    ac.get_pen_color(CID=666)
    ac.hand_up(CID=666)
    ac.hand_down(CID=666)
